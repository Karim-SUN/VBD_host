...
class VBDTest(VBD):
    ...
    ################### Testing Setup ###################
    def inference_conditioner(self, batch, intent_command=None) -> Dict[str, torch.Tensor]:
        """
        ...
        if intent_command is None:
            # 默认模式: 无条件预测 (使用 NULL 意图)
            # 假设 NULL 意图索引为 0 (取决于你的 IntentConditioner 定义)
            intent_command = torch.zeros(B, self._agents_len, dtype=torch.long, device=self.device)
        else:
            # 受控模式: 使用用户提供的意图
