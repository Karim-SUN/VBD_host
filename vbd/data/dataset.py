...
class WaymaxDataset(Dataset):
    ...
    def gen_tensor(self, data):
        ...
        T_future = agents_future.shape[1]

        tensors = {
            "agents_history": torch.from_numpy(agents_history),
            "agents_history_norm": torch.from_numpy(agents_history - agents_current.repeat(T_history, axis=1)),
            "agents_interested": torch.from_numpy(agents_interested),
            "agents_future": torch.from_numpy(agents_future),
            "agents_future_norm": torch.from_numpy(agents_future - agents_current.repeat(T_future, axis=1)[..., :5]),
