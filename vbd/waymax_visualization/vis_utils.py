...
def plot_traj_with_time(
    ...
    for obj_type, traj in zip(obj_type_list, trajs):
        ...
        lc = LineCollection(segments, cmap='cool', norm=norm, linestyle=linestyle, alpha=alpha, zorder=3)
        # Set the values used for colormapping
        lc.set_array(valid_time)
        lc.set_linewidth(linewidth)
        line = ax.add_collection(lc)

    fig.colorbar(line, ax=ax, label='TimeStep (0.1s)', location='right', shrink=0.3, pad=0.02)
