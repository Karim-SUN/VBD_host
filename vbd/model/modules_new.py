...
class GoalPredictor(nn.Module):
    def __init__(self, future_len=80, action_len=5, agents_len=32):
        ...
        self.type_encoder = nn.Embedding(10, 256, padding_idx=0)
    ...
    def forward(self, inputs):
        ...
        query = encodings[:, :self._agents_len, None]

        num_batch, num_agents, _, _ = query.shape
        ...
        for i in range(self._agents_len):
            ...
            actions.append(self.act_decoder(query_content).reshape(
                num_batch, self._future_len // self._action_len, 2
            ))
            types.append(self.type_decoder(query_content).squeeze(-2))

        actions = torch.stack(actions, dim=1)
        types = torch.stack(types, dim=1)
