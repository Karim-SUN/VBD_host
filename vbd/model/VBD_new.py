...
from .modules_new import Encoder, Denoiser, GoalPredictor
from .utils import DDPM_Sampler
from .model_utils_new import inverse_kinematics, roll_out, batch_transform_trajs_to_global_frame, get_trajectory_type
...
class VBD(pl.LightningModule):
    ...
    def forward_predictor(self, encoder_outputs):
        ...
        goal_trajs = roll_out(current_states, goal_actions,
                              action_len=self.predictor._action_len, global_frame=True)
    ...
    def forward_and_get_loss(self, batch, prefix = '', debug = False):
        ...
        agents_future_valid = agents_future_valid[:, :, 1].unsqueeze(-1).expand_as(agents_future_valid) & agents_future_valid
        ...
        if self._train_predictor:
            ...
            # pred_loss = goal_loss_mean + 0.05 * score_loss_means
