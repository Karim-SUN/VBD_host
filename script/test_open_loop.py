...
def run_simulation(args):
    ...
    # Save results
    SAVE_PATH = f'testing_results/test_{args.test_mode}/{args.name}'
    ...
    for scenario_id, scenario in data_iter:  
        ...
        for t in (range(initial_state.remaining_timesteps)):
            # i = t % args.replan
            i = t % N_SIMULATION_STEPS

            # if i == 0:
            if t == 0:
                # print("Replan at ", current_state.timestep)
                print("Plan at ", current_state.timestep)

                with torch.no_grad():
                    sample = dataset.process_scenario(current_state, current_state.timestep, use_log=True)
                    ...
                    if args.test_mode == 'diffusion':
                        ...
                        pred_traj = batch['agents_future'][0, :, :-1, :]
        ...
        # if scenario_id == '52568d00b88a3752':
        #     pass

        if args.save_simulation:
            # Get the first state and save it as an image
            first_state = log_states[1]
            first_state.sim_trajectory.pred_trajs = pred_traj
            img = plot_state(first_state)

            # Save the first frame as an image using mediapy
            mediapy.write_image(os.path.join(SAVE_PATH, f'{scenario_id}_first_frame.png'), img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_mode', type=str, default='gt')
